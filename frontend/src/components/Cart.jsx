...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        const fetchCartProducts = async () => {
            try {
                const response = await fetch(`http://localhost:4000/api/carts/${cartId}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleDeleteProduct = async (productId) => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/carts/${cartId}/product/${productId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    return (
        <>
            ...
            {cartProducts.map((product, index) => (
                <div key={index}>
                    <img src={`http://localhost:4000/uploads/products/${product.thumbnail}`} alt={product.title} />
                    <p>{product.title}</p>
